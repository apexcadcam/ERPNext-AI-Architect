...
3. **Closed in Sprint 24 — two repositories.** At `v1.3.0`, `CanonicalRepository` held `frappe` and `erpnext`. `hrms` was admitted in Sprint 24 with its required supporting-corpus closure `{erpnext, frappe}`; the enum remains closed and default-deny, and arbitrary Frappe applications are still not supported. See [W3](docs/evidence-platform/BACKLOG.md#w3--hrms-support) and the [Sprint 24 Release Notes](SPRINT24_RELEASE_NOTES.md).
